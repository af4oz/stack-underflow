...
const useModal = <RefType extends HTMLElement>({
   ...
   */
  const avoidFocusOnMouseDownRef = useRef(false);

  useEffect(() => {
    function handleMouseDown(e: MouseEvent) {
      avoidFocusOnMouseDownRef.current = true;
    ...
    function handleMouseUp(e: MouseEvent) {
      ...
      avoidFocusOnMouseDownRef.current = false;
    }
  ...
  useEffect(() => {
    ...
    const trapFocus = (e: FocusEvent) => {
      if (avoidFocusOnMouseDownRef.current) return;
